 ...
 * @internal
 */
export function createQuickDialogConfig(context: ContextComplete): IQuickDialogConfig {
    return {
      appId: context.app.id,
      isContent: context.app.isContent,
      isInnerContent: context.instance.id !== context.contentBlockReference.id, // if it differs, it's inner
      hasContent: context.app.hasContent,
      isList: context.contentBlock.isList,
      templateId: context.contentBlock.templateId,
      contentTypeId: context.contentBlock.contentTypeId,
      // user: ContextOfUser.fromContext(context);
      supportsAjax: context.app.supportsAjax,
      debug: window.$2sxc.debug.load
    } satisfies IQuickDialogConfig;
}