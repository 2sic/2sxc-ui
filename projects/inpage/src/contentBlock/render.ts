...
class RendererGlobal extends HasLog {
    ...
    reloadAndReInitialize(context: ContextComplete, triggeredBy: string, forceAjax?: boolean, preview?: boolean): Promise<void> {
        const cl = this.log.call('reloadAndReInitialize', `..., triggeredBy: ${triggeredBy}, forceAjax: ${forceAjax}, preview: ${preview}`, undefined, { context: context });
        ...
        const finalPromise = promiseChain.then((wfArgs): Promise<void> => {
            ...
            return this.ajaxLoad(context, C.ContentBlock.UseExistingTemplate, preview ?? false, 'render.reloadAndReInitialize')
                .then((result) => {
