...
function waitToWatchFolder(path, cpxFilter, targetAddOn) {
  const lastSlash = path.lastIndexOf('/');
  const parentPath = path.substr(0, lastSlash);
  const folderToWaitFor = path.substr(lastSlash + 1, 25);
  console.log(chalk.blue(`Watch to Copy '${parentPath}', '${folderToWaitFor}', '${cpxFilter}', '${targetAddOn}'`));
  ...
  watcher
    .on('addDir', path => { 
        ...
        setTimeout(createCallbackToRunAllCpx(cpxFilter, targetAddOn, true), 1000);
        // callback();
...
module.exports.watchToCopy = waitToWatchFolder;
module.exports.createCallbackToRunAllCpx = createCallbackToRunAllCpx;